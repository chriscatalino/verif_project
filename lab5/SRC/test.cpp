	...
	if(posRow<1 || posCol<1 || posRow>=(int)(DIBH.dHeight-1) || posCol>=int(DIBH.dWidth-1)){
		...
		for(tRow=-1; tRow<=1; tRow++){
			for(tCol=-1; tCol<=1; tCol++){
				...
				// Read the Gradient image from the memory, pixel wise, Notice the Offset!!!
				 dBlockA3x3[tRow+1][tCol+1] = Read_Pixel_from_Mem(posRow+tRow, posCol+tCol,dOffsetBlock*2);
		...
		for(tRow=-1; tRow<=1; tRow++){
			for(tCol=-1; tCol<=1; tCol++){
				...
				// Code Added by Chandrahas starts here
				// Read the Direction image from the memory, pixel wise, Notice the Offset!!!
				 dBlockB3x3[tRow+1][tCol+1] = Read_Pixel_from_Mem(posRow+tRow, posCol+tCol,dOffsetBlock*3);
				// Code Added by Chandrahas ends here 
			}
		}
		// Read 3x3 block from Memory
		for(tRow=-1; tRow<=1; tRow++){
			for(tCol=-1; tCol<=1; tCol++){
				...
				// Code Added by Chandrahas starts here
				// Read the Unprocessed Edge image from the memory, pixel wise, Notice the Offset!!!
				 dBlockC3x3[tRow+1][tCol+1] = Read_Pixel_from_Mem(posRow+tRow, posCol+tCol,dOffsetBlock*4);
				// Code Added by Chandrahas ends here 
			}
		}
		wait(1);
		...
		// Code added by Chandrahas, starts here
		UINT IDCANNY = 4; // Canny Edge detector identified by ID of 4
		bool bCE = 1; // disable the Chip till we prepare the Address word
		bool bWE = 0; // Because we are writing to Canny Edge Detector
		UINT Addr; // I dont know about this, have to lookup
		UINT AddressOut=0; // Initializing the Address to 0, before we writing something sensible to it!
		bOPEnable = 1; // Disable the Operation until we load the values into registers of the Canny Edge Detector
		...
		// Code added by Chandrahas starts here
		dWriteReg = WRITE_REGX; // Because we wanted to write to RegX
		...
		// Code added by Chandrahas starts here
		dWriteReg = WRITE_REGY; // Because we wanted to write to RegY
		for(tRow=0; tRow<3; tRow++){
			for(tCol=0; tCol<3; tCol++){
		...
		// Code added by Chandrahas starts here
		dWriteReg = WRITE_REGZ; // Because we wanted to write to RegZ
		for(tRow=0; tRow<3; tRow++){
			for(tCol=0; tCol<3; tCol++){
		...
		// Code Added by Chandrahas starts here
		wait(1);	AddressBus.write(BITOFF(AddressOut, IDX_CANNY_bOPEnable)); // Making bOPEnable = 0		
		wait(4);	AddressBus.write(BITON(AddressOut, IDX_CANNY_bOPEnable)); // Making bOPEnable =1, after wait for operation to complete
		// Code Added by Chandrahas ends here
		...
		// refer to Do_5x5_Gaussian or Do_3x3_Sobel
		// Code Added by Chandrahas starts here
		dReadReg = REG_HYSTERESIS ; // Because we needed the output of hysteresis operation
		tRow = 1; 	tCol = 1; // Doesnt matter, but just kept it like this, Needs Review 	
		bWE = 1;	bCE = 1; // Because we are reading from Canny
		AddressOut = (IDCANNY << 28)+(bOPEnable << 27)+(OPMode << 24)+(dWriteReg << 20)+(dReadReg << 16)+(tRow<<5)+(tCol<<2)+(bWE<<1)+bCE;
		AddressBus.write(AddressOut);
		DataBus.write("ZZZZZZZZ");
		wait(1);	AddressBus.write(BITOFF(AddressOut, IDX_CANNY_bCE)); // Make bCE zero and read from Canny
		wait(4);	UINT tValue = DataBus.read().to_uint(); // Store the read value in tValue variable
		wait(1);	AddressBus.write(BITON(AddressOut, IDX_CANNY_bCE)); // Make bCE 1 and stop the operation performed
		wait(1);	//ReleaseBus();
		// Code Added by Chandrahas ends here	
		...
		// Code Added by Chandrahas starts here
		Init_to_Mem();
		Send_Pixel_to_Mem(posRow, posCol, tValue, dOffsetBlock*4); // Send the tValue to Memory at given posRow & posCol
		DataBus.write("ZZZZZZZZ");
		// Code Added by Chandrahas ends here
	}
}
...
void test::Do_3x3_NMS(int posRow, int posCol)
{
	...
	if(posRow<1 || posCol<1 || posRow>=(int)(DIBH.dHeight-1) || posCol>=int(DIBH.dWidth-1)){
		...
		// (Setting Control variables)
		// Code Added by Chandrahas starts here
		IDCANNY = 4; // ID of 4 for Canny Edge detector  		
		bWE = 0; // For Write Mode
		bCE = 1; // Disable the Chip until we assign all the needed variables 
		bOPEnable = 1; // Disable all the Operations, Until we load the data into the registers  
		// Code Added by Chandrahas ends here
		...
		// b. Send dBlockA3x3 to Canny
		// Code Added by Chandrahas starts here
		dWriteReg = WRITE_REGX; // Because we are writing to RegX
		for(tRow=0; tRow<3; tRow++){
			for(tCol=0; tCol<3; tCol++){
				DataBus.write(dBlockA3x3[tRow][tCol]);
				AddressOut = (IDCANNY << 28)+(bOPEnable << 27)+(OPMode << 24)+(dWriteReg << 20)+(dReadReg << 16)+(tRow<<5)+(tCol<<2)+(bWE<<1)+bCE;
		...
		// b. Send dBlockB3x3 to Canny
		// Code Added by Chandrahas starts here
		dWriteReg = WRITE_REGY; // Because we are writing to RegY
		for(tRow=0; tRow<3; tRow++){
			for(tCol=0; tCol<3; tCol++){
				DataBus.write(dBlockB3x3[tRow][tCol]);
				AddressOut = (IDCANNY << 28)+(bOPEnable << 27)+(OPMode << 24)+(dWriteReg << 20)+(dReadReg << 16)+(tRow<<5)+(tCol<<2)+(bWE<<1)+bCE;
		...
		for(tRow=0; tRow<3; tRow++){
			for(tCol=0; tCol<3; tCol++){
				...
				// Insert here // ---------------------------------------------------------
				// refer to Do_5x5_Gaussian or Do_3x3_Sobel
				// Prepare the relevant Address Word and Write it
				// Code Added by Chandrahas starts here
				AddressOut = (IDCANNY << 28)+(bOPEnable << 27)+(OPMode << 24)+(dWriteReg << 20)+(dReadReg << 16)+(tRow<<5)+(tCol<<2)+(bWE<<1)+bCE;
				AddressBus.write(AddressOut);
				DataBus.write("ZZZZZZZZ");
				wait(1);	AddressBus.write(BITOFF(AddressOut, IDX_CANNY_bCE)); // Make bCE = 0, so that it reads 
				wait(4);	dBlockC3x3[tRow][tCol]= DataBus.read().to_uint(); // Capture the read data from Canny
				wait(1);	AddressBus.write(BITON(AddressOut, IDX_CANNY_bCE)); // Make bCE = 1, to stop operation
				wait(1); // Just wait for things to settle, before you proceed furthur
				// Code Added by Chandrahas ends here 
...
void test::do_test(){
	...
	while(true){
		...
		unsigned int k,l; // integer types changed by Chandrahas to remove warnings
