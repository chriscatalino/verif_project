  ...
  void UART_XMTR::Send_bit() 
  {
  	switch(IntState)
	{
		case STATE_IDLE:
			...
			{
				XMT_datareg = Data_Bus.read().to_uint(); // Copy off the data from Data Bus to XMT_datareg
				//cout << "@ " << sc_time_stamp() << " INFO: The Data read from the Data Bus is " << Data_Bus.read().to_uint() << "\n";
			}
